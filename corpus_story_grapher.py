...
def convert_to_doc(predictor, text):
    doc = predictor(text)
    return doc
...
def main():
    ...
    # Put data path here
    data_path = "/mnt/pccfs/not_backed_up/data/bookcorpus/books_large_total.txt"
    ...
    save_file = save_path + 'book_corpus_triples.csv'
    ...
    # Get triples from line
    # Save append triples to the csv file
    ...
    for line in file:
        ...
        if any(([True if indicator in line else False for indicator in TITLE_INDICATORS])):
            print("Line is likely a title. Skipping to next line")
            continue
        ...
        for sent in doc.sents:
            ...
            # Trim the triple
            trimmed_triple = trim_triple(spacy_sent, relevant_triple) 
            print("Trimmed Triple:", trimmed_triple, "\n")

            if None not in trimmed_triple:
                triple_count += 1
                print("Saving triple to file.")
                print(triple_count, " triples have been extracted so far.")
               
                with open(save_file, mode="a") as saveFile:
                    triple_writer = csv.writer(saveFile, delimiter=',',quoting=csv.QUOTE_ALL)
                    triple_writer.writerow([sent.text, trimmed_triple])
                    print("Successfully saved triple to file")
